...
public class FutureTaskCalisma3 {
    ...
    public static int amountOfDivisibleByFutureAndShutdown(final int first, final int last, final int divisor)
            throws InterruptedException, ExecutionException {
    	...
    	TaskWithLongSleepImpl task1Impl = new TaskWithLongSleepImpl(first, last / 2, divisor);
    	TaskWithLongSleepImpl task2Impl = new TaskWithLongSleepImpl(first, last / 2, divisor); 
    	TaskWithLongSleepImpl task3Impl = new TaskWithLongSleepImpl(first, last / 2, divisor);
  	  	try {
			executor.submit(task1Impl);
			executor.submit(task2Impl);
			executor.submit(task3Impl);
			System.out.println("Active Task Count=" + ((ThreadPoolExecutor) executor).getActiveCount() + " Task Count=" + ((ThreadPoolExecutor) executor).getTaskCount());
...
//        timeStartFuture = Calendar.getInstance().getTimeInMillis();
//        resultFuture = FutureTaskCalisma.amountOfDivisibleByFutureWithSyncQueueSize(0, MAX_NUMBER, 3);
//        timeEndFuture = Calendar.getInstance().getTimeInMillis();
//        timeNeededFuture = timeEndFuture - timeStartFuture;
//        System.out.println("Result (Future InvokeTimeout): " + resultFuture + " calculated in " + timeNeededFuture + " ms");
    	
//    	timeStartFuture = Calendar.getInstance().getTimeInMillis();
//    	resultFuture = FutureTaskCalisma3.amountOfDivisibleByFutureWithSyncQueueSizeWithSleep(0, MAX_NUMBER, 3);
//      	timeEndFuture = Calendar.getInstance().getTimeInMillis();
//      	timeNeededFuture = timeEndFuture - timeStartFuture;
//      	System.out.println("Result (Future InvokeTimeout): " + resultFuture + " calculated in " + timeNeededFuture + " ms");
      	...
      	timeStartFuture = Calendar.getInstance().getTimeInMillis();
    	resultFuture = FutureTaskCalisma3.amountOfDivisibleByFutureAndShutdown(0, MAX_NUMBER, 3);
      	timeEndFuture = Calendar.getInstance().getTimeInMillis();
      	timeNeededFuture = timeEndFuture - timeStartFuture;
      	System.out.println("Result (Future InvokeTimeout): " + resultFuture + " calculated in " + timeNeededFuture + " ms");
